...
Additionally, we provide a home-made GUI based application called meadowlarkFeeder to control the Meadowlark SLM model used in the paper (developed by Vincent de Sars). The software is based on the SDK provided by the manufacturer has an optimized graphic interface and some additional features that ease the SLM adressing for photostimulation pipelines.
